...
import { NeutrinoService } from 'src/services/neutrino/neutrino.service';
...
export class UsersController {
    ...
    async getUser(@Res() res, @Param() param) {
        if (param && mongo.ObjectID.isValid(param.id)) {
    ...
    async create(@Res() res, @Body() user: User) {
        if (user) {
            if (User.checkUser(user)) {
                ...
                res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
                    code: 135,
                    message: 'object malformed error',
                    success: false
                });
            }
        } else {
            res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
                code: 136,
                message: 'not object provided error',
                success: false
            });
        }
    }
    ...
    @Post('contacts')
    async createContacts(@Res() res, @Body() contact, @Req() req) {
        console.log(JSON.stringify(contact));if (contact) {
    ...
    async getContactsBetweenUsers(@Res() res, @Param() params) {
        if (params && mongo.ObjectID.isValid(params.userId1) && mongo.ObjectID.isValid(params.userId2)) {
